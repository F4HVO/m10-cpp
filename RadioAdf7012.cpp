...
void RadioAdf7012::adf_reset_register_two(void) {
    ...
    // +- 10kHz : 82, +- 5KHz : 40
    adf_config.r2.modulation_deviation = 40;                    // FSK : deviation as a multiple of Fpfd/2^14, with Fpfd = Fxtal/R
...
void RadioAdf7012::send_data( uint8_t data[], uint32_t size, int headerSize )
{
    ...
    while ( i < size )
    {
        // Do not manchesterize header
        if ( i < headerSize )
        {
            // prepare bit
            bitToSend = ( ( data[i] >> j ) & 1 ) ;
            --j ;
            if ( j < 0 )
            {
                j = 7 ;
    ...
    // Wait one last time (value to send does not matter)
    waitAndSendBit( bitToSend ) ;
    waitAndSendBit( bitToSend ) ;
    waitAndSendBit( bitToSend ) ;
